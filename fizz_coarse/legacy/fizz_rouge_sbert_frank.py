...
parser.add_argument('--output_path', type=str, default='results_coarse/fizz_rouge_sbert_frank.json')
...
def main_no_discards_coarse():  # not discarding selected document sentences
    # 입력 파일 로드 (JSON 형식)
    with open(args.input_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    output_path = args.output_path.replace(".json", "_no_discards_coarse.json")
    # JSON 파일로 저장
