...
class LLMAggregator:
    ...
    def get_prompt(self, text):
        if self.model_name == "orca2":
            return prompts_aggregate.orca_format(text)
    ...
    def orca_format_with_truncation_handling(self,text, tokenizer, max_length=2048, min_shot=1, max_shot=10):
        
        for num_shot in range(max_shot, min_shot - 1, -1):
            prompt = prompts_aggregate.orca_format(text, num_shot=num_shot)
            
            input_ids = tokenizer(prompt, return_tensors="pt")["input_ids"]
            if input_ids.shape[1] <= max_length:
                return prompt
        # fallback to 1-shot if nothing fits
        return prompts_aggregate.orca_format(text, num_shot=1)
    ...
    def aggregate(self, sum_texts, doc_texts, scores):
        ...
        for summary, docs, scs in zip(sum_texts, doc_texts, scores):
            ...
            # Truncation-safe few-shot prompt 생성
           
            full_prompt = self.orca_format_with_truncation_handling(user_prompt, self.tokenizer)
            ...
            print(decoded)
            try:
                # ✅ 안전한 응답 추출: 마지막 assistant 응답만 파싱
