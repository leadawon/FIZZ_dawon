...
parser.add_argument('--input_path', type=str, default='data/xsumfaith.json')
parser.add_argument('--output_path', type=str, default='results_align/fizz_rouge_sbert_xsumfaith.json')
...
parser.add_argument('--weight_rouge', type=float, default=0.5, help='Weight for ROUGE score')
parser.add_argument('--weight_bert', type=float, default=0.5, help='Weight for BERTScore')
...
def main_no_discards_align():  # not discarding selected document sentences
    # 입력 파일 로드 (JSON 형식)
    with open(args.input_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for entry in tqdm(data, desc="Processing", mininterval=0.01):
        ...
        modified_sentences = []
        for sentence in doc_sentences:
            modified_sentences.append((sentence, "original"))  # 선택된 문장도 먼저 추가
            if sentence in selected_sentences:
                atomic_facts = decomposer.atomic_facts_decompose(sentence)
                filtered_facts = filterer.atomic_facts_filtering(sentence, atomic_facts)
                fact_sentences = decomposer.split_sentences(filtered_facts)
                for fact_sentence in fact_sentences:
                    modified_sentences.append((fact_sentence, "fact"))

        # 5. 순서 보존된 문서 재구성
        final_doc = ' '.join([sent[0] for sent in modified_sentences])

        # 6. Score 계산
        score = scorer.atomic_facts_scoring(final_doc, filtered_atomic_facts, modified_sentences)
    ...
    output_path = args.output_path.replace(".json", "_no_discards_align.json")
    # JSON 파일로 저장
